...
| [6.5 流回调](https://face2ai.com/CUDA-F-6-5-%E6%B5%81%E5%9B%9E%E8%B0%83/) | `38_stream_call_back` | `cudaLaunchHostFunc` 是把 host 函数排进 stream 的完成通知机制；host function 中不能随意调用 CUDA API。 | 让用户预测 host function 打印顺序是否等于 stream 创建顺序，并解释异步完成顺序。 |
...
本仓库只支持 `sm_80`、`sm_86`、`sm_87`、`sm_89`、`sm_90`。如果指定范围，统一写全 `80;86;87;89;90`；如果只指定一代，统一使用 `90` / `sm_90`。设备对应关系只作为识别参考：A100 为 `80`，RTX 3090 为 `86`，Jetson AGX Orin 为 `87`，L40/L4/4090 为 `89`，H100/H200 为 `90`。如果全量构建太慢，先进入单章目录用 `nvcc` 或构建对应 target。
...
学习目标：掌握 H2D、kernel、D2H 的流水化；理解 depth-first 与 breadth-first 提交顺序；观察异步 API 和 stream host function。

授课指导：用时间线画出每个 segment 在每个 stream 里的 H2D/kernel/D2H。让用户分别调大 `N_SEGMENT`、`N_REPEAT`，观察什么时候 overlap 明显。强调异步拷贝需要 pinned host memory 才有意义；第 38 章使用 `cudaLaunchHostFunc` 观察 stream 完成通知。
...
```bash
nvcc -arch=sm_90 13_nested_hello_world/nested_Hello_World.cu -o /tmp/nested_Hello_World -lcudadevrt --relocatable-device-code true
```
...
如果编译失败且提示架构不支持，指导用户根据 `nvidia-smi` 和 GPU 型号设置范围 `CMAKE_CUDA_ARCHITECTURES="80;86;87;89;90"`；如果只指定单代架构，统一使用 `90`。
